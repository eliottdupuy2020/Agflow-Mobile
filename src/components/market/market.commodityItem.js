...
class MarketCommodityItemComponent extends React.Component {
  onCommodityItemPress = (commodityData) => {
    this.props.onItemPress(commodityData);
  };

  render() {
    const {themedStyle, commodityData} = this.props;

    const priceList = [];
    for (let i = 0; i < 4; i++) {
      if (commodityData.aggregates[i]) {
        const aggregate = commodityData.aggregates[i];
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => {this.onCommodityItemPress({id: commodityData.id, title: commodityData.commodity})}}
      >
        <View style={themedStyle.commodityImageContainer}>
          <Avatar
            style={themedStyle.commodityImage}
            source={{uri: commodityData.icon}}
            size='medium'
            shape='rounded'
          />
        </View>
        <View style={themedStyle.commodityDetailContainer}>
          <View style={themedStyle.commodityNameContainer}>
            ...
            <Text style={themedStyle.commodityInfo}>{commodityData.destination} / {commodityData.incoterm}</Text>
          </View>
...
export const MarketCommodityItem = withStyles(MarketCommodityItemComponent, (theme) => ({
  container: {
    flex: 1,
    flexDirection: 'row',
    paddingVertical: 8,
